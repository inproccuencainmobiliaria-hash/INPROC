#!/usr/bin/env python3
"""Genera public/intro_audio.wav para la intro IntroInproc (5.0s, 48kHz estéreo).
...
Capas:
  ...
  1.0-3.8s   se suma una capa a 220Hz + ruido rosa filtrado muy tenue,
             ambos con una rampa ascendente de volumen lenta
  3.8-4.7s   riser: barrido de frecuencia ascendente
  4.7-5.0s   golpe grave (sub-bass) con pitch descendente y decaimiento rápido
...
OUT_PATH = "public/intro_audio.wav"
TARGET_LUFS = -14.0
...
# --- Capa 4: riser 3.8-4.7s, barrido ascendente -----------------------------
riser_env = envelope(t, [(3.8, 0.0), (3.85, 0.22), (4.68, 1.0), (4.70, 0.0)])
riser_local_t = np.clip(t - 3.8, 0.0, 0.9)
riser_tone = signal.chirp(riser_local_t, f0=180, t1=0.9, f1=1800, method="logarithmic")
riser = riser_tone * riser_env

# --- Capa 5: golpe grave (sub-bass) 4.7-5.0s --------------------------------
sub_local_t = np.clip(t - 4.7, 0.0, None)
sub_freq = 38 + 70 * np.exp(-sub_local_t / 0.045)
sub_phase = 2 * np.pi * np.cumsum(sub_freq) / SR
attack = np.clip(sub_local_t / 0.003, 0.0, 1.0)
decay = np.exp(-sub_local_t / 0.11)
sub_amp = attack * decay * (t >= 4.7)
