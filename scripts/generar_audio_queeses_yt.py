#!/usr/bin/env python3
"""Genera public/audio_queeses_yt.wav para la intro IntroQueEsYT (4.0s, 48kHz estéreo).

Todo el audio se sintetiza con numpy/scipy — no se descarga ni referencia
ningún archivo externo. Misma técnica que scripts/generar_audio_queeses.py,
con la línea de tiempo específica de la versión YouTube (16:9):

  0.0-1.0s   zumbido grave (55Hz + 110Hz), fade-in, volumen bajo
  1.0-2.5s   se suma una capa a 220Hz + ruido rosa filtrado tenue,
              ambos con una rampa ascendente de volumen
  2.5-3.7s   riser: barrido de frecuencia ascendente
  3.7-4.0s   golpe grave (sub-bass) con decaimiento rápido que cierra

Nota: usa un nombre de archivo distinto al de la composición IntroQueEs
(vertical) a propósito -- esa ya tiene su propio public/audio_queeses.wav
con otra sincronización (riser 3.3-3.7s), y sobrescribirlo rompería ese
video ya entregado.
...
OUT_PATH = "public/audio_queeses_yt.wav"
TARGET_LUFS = -14.0
...
hum_env = envelope(t, [(0.0, 0.0), (1.0, 0.16), (2.5, 0.28), (3.5, 0.10), (3.75, 0.0)])
hum = (0.6 * np.sin(2 * np.pi * 55 * t) + 0.4 * np.sin(2 * np.pi * 110 * t)) * hum_env

# --- Capa 2: 220Hz que se suma a partir de 1.0s -----------------------------
layer_220_env = envelope(t, [(1.0, 0.0), (1.3, 0.22), (2.5, 0.4), (2.6, 0.0)])
layer_220 = np.sin(2 * np.pi * 220 * t) * layer_220_env

# --- Capa 3: ruido rosa filtrado, tenue --------------------------------------
pink_env = envelope(t, [(1.0, 0.0), (1.4, 0.08), (2.5, 0.14), (2.6, 0.0)])
...
# --- Capa 4: riser 2.5-3.7s, barrido ascendente -----------------------------
riser_env = envelope(t, [(2.5, 0.0), (2.55, 0.18), (3.68, 1.0), (3.70, 0.0)])
riser_local_t = np.clip(t - 2.5, 0.0, 1.2)
riser_tone = signal.chirp(riser_local_t, f0=160, t1=1.2, f1=1900, method="logarithmic")
